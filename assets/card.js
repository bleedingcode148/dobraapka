...
var adress1 = data['adress1'] || '';
var adress2 = data['adress2'] || '';
var city = data['city'] || '';
var adressText = '';
if (adress1) {
    adressText = "ul. " + adress1;
    if (adress2 || city) {
        adressText += "<br>";
        if (adress2) adressText += adress2;
        if (adress2 && city) adressText += " ";
        if (city) adressText += city;
    }
}
setData("adress", adressText || '');

// Wypełnij dane mDowodu
if (data['mdow_series']) {
  var mdowElement = document.getElementById("mdow_series_display");
  if (mdowElement) mdowElement.innerHTML = data['mdow_series'];
}
if (data['issue_date']) {
  var issueElement = document.getElementById("issue_date_display");
  if (issueElement) issueElement.innerHTML = data['issue_date'];
}
if (data['expiry_date']) {
  var expiryElement = document.getElementById("expiry_date_display");
  if (expiryElement) expiryElement.innerHTML = data['expiry_date'];
}
if (data['father_name']) {
  var fatherElement = document.getElementById("father_name_display");
  if (fatherElement) fatherElement.innerHTML = data['father_name'];
}
if (data['mother_name']) {
  var motherElement = document.getElementById("mother_name_display");
  if (motherElement) motherElement.innerHTML = data['mother_name'];
}

// Użyj home_date z URL jeśli jest dostępny, w przeciwnym razie użyj localStorage
var homeDateValue = data['home_date'];
if (homeDateValue) {
  document.querySelector(".home_date").innerHTML = homeDateValue;
} else {
